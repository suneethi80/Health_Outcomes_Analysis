...
```{r}
tn_county<-gdp_df %>% 
  mutate(Country= str_replace(Country, "C̫te d'Ivoire", "Ivory Coast")) %>% 
"de kalb"
```
...
mortality<-tn_2019_healthtotal %>% 
  select(County, Alcohol_drivg_death, Drivg_deaths_no, `Age-Adjusted Mortality`, `Child Mortality Rate`, `Infant Mortality Rate`, `Drug Overdose Mortality Rate`, `MV Mortality Rate`, `Homicide Rate`, `Firearm Fatalities Rate`, Rural_pct)
...
pivot_longer(cols=c(Alcohol_drivg_death, Drivg_deaths_no, `Age-Adjusted Mortality`, `Child Mortality Rate`, `Infant Mortality Rate`, `Drug Overdose`, `Motor Vehicle`, `Homicide`, `Firearm`), names_to = "Deaths", values_to = "value")
